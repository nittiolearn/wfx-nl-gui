...
// Helpers
function _pt(lesson) {
    return lesson.playerServices.nlPageType;
}
...
function _Page_init(self, lesson, oPage) {
    self.lesson = lesson;
    self.oPage = oPage;
    self.pt = _pt(lesson).createNew(oPage);
    self.sections = [];
...
function nlesson() {
	...
	function _setCustomLayout(oPage, layout) {
		if (layout.length == 0) {
	...
	function Lesson_addPage(pageType, customLayout) {
		...
		_setCustomLayout(oPage, customLayout);
	...
	function Lesson_changePageType(pageType, customLayout) {
		...
		_setCustomLayout(curPage.oPage, customLayout);
