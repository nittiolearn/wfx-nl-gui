...
function _showPgSecView(section) {
    section.showElem = {templ: false, text: false, view: true, line: true};
}

function _showPgSecText(section) {
    var pageCtx = _getPageCtx(section.page);
    if (pageCtx == 'edit_templ') {
        section.showElem = {templ: true, text: false, view: false, line: false};
        return;
    }

    var template = section.getTemplateFromEditor();
    var sectionTemplate = new njs_lesson_helper.SectionTemplate(template, section); // TODO-MUNNI

    if (sectionTemplate.getMode() == 'text' && !sectionTemplate.isSpecialText()) {
        section.showElem = {templ: false, text: true, view: false, line: false};
    } else {
        section.showElem = {templ: false, text: false, view: true, line: true};
    }
}
...
function _showPgSecTextAndLines(section) {
    section.showElem = {templ: false, text: true, view: false, line: true};
}

console.log('TODO-MUNNI - review from here - line 338');
...
// Note: if max score of any page type is changed, please review the _pageTypeToMaxScores dict
// in mlesson.py
//-------------------------------------------------------------------------------------------------
var PageTypes = [
