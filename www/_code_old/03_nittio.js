...
nittio = function() {
	...
	function dateToString(dateStr) {
		...
		if (isNaN(d.valueOf()))
			return dateStr;
		return njs_helper.fmt2('{}-{}-{} {}:{} Hrs', d.getFullYear(), _pad2(d.getMonth() + 1), _pad2(d.getDate()), _pad2(d.getHours()), _pad2(d.getMinutes()));
	}
	...
	function _fmtMinutes(min) {
		...
		var ret = '';
		if (day > 0)
			ret += _fmtTime(day, 'day') + ' ';
		if (hr > 0)
			ret += _fmtTime(hr, 'hour') + ' ';
		if (min > 0 || ret == '')
			ret += _fmtTime(min, 'minute');
		return ret;
	}
	...
	function areArraysEqual(array1, array2, arrayElementCompareFunction) {
		...
		for (var i = 0; i < array1.length; i++) {
			if (!arrayElementCompareFunction(array1[i], array2[i]))
				return false;
		}
		return true;
	}
	...
	var DateTimePickerMethods = {
		...
		adustDateIfNeeded : function(inputDate, pickTime) {
			if (inputDate == null || inputDate.constructor !== Number)
				return inputDate;
			var ret = new Date();
	...
	function bindHotkey(bindOn, bindFilter, hotkey, hotkeyFn, propogate) {
		...
		jQuery(bindOn).bind(_getKeyEvent(bindFilter), hotkey, function(event) {
			...
			// Needed because of a firefox bug
			return propogate;
		});
	}
	...
	function closePopupsAndDlgs(event) {
		...
		if (ret > 0)
			njs_helper.stopPropagation(event);
	}
	...
	function redirDelay(nextUrl, delay, disableLeaveCheck) {
		...
		if (nextUrl == 'self') {
			...
			// Go to start page now!. Will reload on timeout
			bSelf = true;
		}
	...
	function resizeImagesToAspectRatio(elem) {
		if (elem.hasClass('aspect_wrt'))
			_cleanupBackgroundAndStyling(elem);
	...
	function initSlides() {
		...
		if (slides.length < 1 || pgNo.length < 1 || navLeft.length < 1 || navRight.length < 1)
			return;
	...
	function _initPage(retainAspect) {
		...
		jQuery(function() {
			...
			if (g_slides != null)
				g_slides.activate(window.location.hash);
